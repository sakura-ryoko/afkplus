...
import com.sakuraryoko.afkplus.impl.player.interfaces.IPlayerInvoker;
...
public class AfkHandler
{
    ...
    @ApiStatus.Internal
    public void registerShadowAfk(int time, String reason)
    {
        if (this.player.isShadowPlayer())
        {
            return;
        }
        ...
        this.player.setShadowPlayer(true);
        this.player.setShadowTimeout(shadowTimeout);
        // This is measuring time, not Epoch time in ms.
        this.player.setAfkTimeMs(Util.getMillis());

        if (reason == null && ConfigWrap.afkMe().defaultShadowReason == null)
        {
            this.player.setAfkReason("<red>none");
        }
        else if (reason == null || reason.isEmpty())
        {
            ...
            Component mess = PlaceholderUtils.parseText(
                    TextHandler.getInstance().formatTextSafe(ConfigWrap.afkMe().shadowStarted),
                    PlaceholderUtils.ofPlayer(this.player.getPlayer()));
        ...
        {
            this.player.setAfkReason(reason);
            String mess1 = ConfigWrap.afkMe().shadowStarted
                    + ConfigWrap.afkMe().shadowPunctuation + reason;
            Component mess2 = PlaceholderUtils.parseText(TextHandler.getInstance().formatTextSafe(mess1), PlaceholderUtils.ofPlayer(this.player.getPlayer()));
            this.sendAfkMessage(mess2);
        }
    ...
    public void unregisterShadowAfk()
    {
        ...
        {
            String ret = ConfigWrap.afkMe().shadowReturned
        ...
        {
            String ret = ConfigWrap.afkMe().shadowReturned + "<r>";
            Component mess1 = TextHandler.getInstance().formatTextSafe(ret);
            Component mess2 = PlaceholderUtils.parseText(mess1, PlaceholderUtils.ofPlayer(this.player.getPlayer()));
            this.sendAfkMessage(mess2);
        }
